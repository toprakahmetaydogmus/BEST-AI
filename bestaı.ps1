...
function Detect-GPU {
    ...
    if ($checkIsWindows) {
        ...
        if ($nvidiaSmi) {
            try {
                # Use --format=csv,noheader,nounits without any spaces after commas (Safer for drivers)
                ...
                if ($nvidiaOut) {
                    ...
                    $global:gpuInfo.Name = $parts[0].Trim()
                    $global:gpuInfo.VRAM = [Math]::Round([double]$parts[1].Trim() / 1024, 1)
        ...
        # AMD kontrol (ROCm / DirectML)
        $gpuName = (Get-WmiObject -Class Win32_VideoController -ErrorAction SilentlyContinue | Select-Object -First 1).Caption
        if ($gpuName -match "AMD|Radeon") {
            $global:gpuInfo.Available = $true
            $global:gpuInfo.Type = "AMD"
            $global:gpuInfo.Name = $gpuName
            Write-OK "AMD GPU algilandi: $gpuName"
        ...
        if ($gpuName -match "Intel") {
            ...
            $global:gpuInfo.Name = $gpuName
            Write-Warn "Intel GPU: $gpuName - CPU modu kullanilacak."
            return
        }
    ...
    elseif ($checkIsLinux) {
        ...
        if ($nvidiaSmi) {
            try {
                # Use --format=csv,noheader,nounits without any spaces after commas (Safer for drivers)
                $nvidiaOut = & nvidia-smi --query-gpu=name,memory.total --format=csv,noheader,nounits 2>/dev/null
                if ($nvidiaOut) {
                    $parts = $nvidiaOut -split ","
                    $global:gpuInfo.Available = $true
                    $global:gpuInfo.Type = "NVIDIA"
...
function Get-InstalledModels {
    try {
        $checkUrl = "http://127.0.0.1:$OLLAMA_PORT/api/tags"
        $resp = Invoke-RestMethod -Uri $checkUrl -TimeoutSec 5
        $global:installedModels = $resp.models | ForEach-Object { $_.name }
    ...
    foreach ($model in $toDownload) {
        ...
        try {
            if ($checkIsWindows) {
                ...
                Invoke-Expression "ollama pull $($model.Name)"
            }
    ...
    <main class="main">
        <div class="topbar">
            ...
            <div class="topbar-right">
                ...
                <span class="chip" id="gpuChip">CPU</span>
            </div>
        </div>
...
'@
    ...
    $htmlContent = $htmlContent.Replace("{{BIND_IP}}", $global:bindIp).Replace("{{OLLAMA_PORT}}", $OLLAMA_PORT).Replace("{{CADDY_PORT}}", $CADDY_PORT).Replace("{{API_KEY}}", $apiKeyMeta)
...
while ($mainChoice -ne "0") {
    ...
    switch ($mainChoice) {
        ...
        "7" {
            ...
            if ($sub -eq "3") { Check-OllamaUpdate }
        }
        "9" {
            Init-Logging
            Uninstall-All
...
Write-Host ""
Write-Host "" 
Write-Host "  Siber Akademi AI Server - Gule gule!" -ForegroundColor Yellow
Write-Host ""
